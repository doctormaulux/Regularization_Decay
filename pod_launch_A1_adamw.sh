#!/usr/bin/env bash
# ============================================================================
# POD A1-adamw — 'large' (66M) from-scratch GPT-2 on WikiText-2, method group: Tau(AdamW-scope)
# ----------------------------------------------------------------------------
...
# Pull with:  bash pod_status.sh --pull     (pods.conf: "adamw <ip> <port> gpt2_large_wikitext_standardized_results")
# ============================================================================
set -euo pipefail
source "$(dirname "$0")/pod_common.sh"
pod_provision
pod_preflight "A1_adamw"
cd "$REPO"
echo "[POD A1_adamw] $(date -u) starting"
python -u gpt2_wikitext_standardized.py --scale large --seeds 10 --instrument \
    --methods "Tau(AdamW-scope)" --quiet 2>&1 | tee -a "logs/pod_A1_adamw.log"
echo "[POD A1_adamw] $(date -u) done -> results/gpt2_large_wikitext_standardized_results.{csv,json} (this group only), results/journal/, results/instrumentation/"
